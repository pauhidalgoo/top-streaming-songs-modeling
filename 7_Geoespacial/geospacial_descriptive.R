...
# VISUALITZACIÓ DELS PUNTS AL MAPA I DE LA DENSITAT DELS PUNTS
...
# Mitjana del seguidors dels artistes de cada país
...
#############
# Visualitzem també la progressió d'aquests artist_followers en els diferents 
# anys que tenim per veure si hi ha diferències en durant el temps
#############
...
# Primer volem veure quin és el gènere més fet a cada país, i després
# veurem el nombre de cançons de cada país que són de cada gènere

# Número de cançons que hi ha al top de cada gènere
...
plot_genre_heatmap <- function(genre) {
  ...
  ggplot(data = data_summarized) +
    ...
    scale_fill_viridis_c(option = "C", na.value = "grey", guide = "colorbar") +
    labs(title = paste("Mapa de Calor del nombre de cançons de", genre),
...
# A continuació mirem quin és el gènere més fet per cada país
...
ploteee <- ggplot(data = data_joined) +
  geom_sf(aes(fill = most_popular_genre), color = "black") +  
  scale_fill_manual(values = genre_colors, na.value = "grey") +  
...
# 1. PERCENTATGE DE CANÇONS DE CERT GÈNERE PER CADA PAÍS
...
# 2. PERCENTATGE DEL TOTAL MUNDIAL DE CANÇONS D'UN GÈNERE PER PAÍS
...
# MAPA INTERACTIU: per veure el nom de l'artista i la seva popularitat
...
# POPULARITAT MITJANA D'UNA CANÇÓ
tm_shape(world) +
  tm_borders() +
...
explicit_percentage_by_country <- filtered_data %>%
  group_by(country) %>%
  summarise(percent_explicit = mean(is_explicit, na.rm = TRUE) * 100) 
...
tm_map <- tm_shape(world) +
  ...
  tm_layout(legend.position = c("left", "bottom"), main.title = "Percentatge de cançons explícites per País")

print(tm_map)

tmap_mode("plot")

# Visualitzem si el país en la seva majoria ha escoltat cançons explícites o no
...
majority_explicit_by_country <- filtered_data %>%
  group_by(country) %>%
  summarise(majority_explicit = mean(is_explicit, na.rm = TRUE) > 0.5)
...
#### VARIABLES NUMÈRIQUES: energy, valence, danceability, speechiness, loudness
...
names <- c("energy", "valence", "danceability", "speechiness", "loudness")

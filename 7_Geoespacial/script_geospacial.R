...
# Asegurando que el dataframe tiene las columnas necesarias para latitud y longitud
data$latitude <- NA  # Añadir columna para latitudes si no existe
data$longitude <- NA # Añadir columna para longitudes si no existe
...
for (i in 1:nrow(unique_locations))  {
  city <- as.character(data$city[i])
  country <- as.character(data$nationality[i])
  city <- URLencode(city)
  country <- URLencode(country)  # Asegúrate que la columna se llama 'nationality'
  ...
  if (status_code(response) == 200) {
    ...
    if (length(dades) > 0) {  # Comprobar que la respuesta contiene datos
      ...
      unique_locations$latitude <- first_proposal$latitude
      unique_locations$longitude <- first_proposal$longitude
    } else {
  ...
  # Pausa para evitar sobrecargar la API (ajusta este tiempo según las políticas de uso de tu API)
  Sys.sleep(1)
}

# Revisar el dataframe actualizado
View(data)
...
for (i in 1:nrow(data)) {
  ...
  if (status_code(response) == 200) {
    ...
    if (length(dades) > 0) {  # Comprobar que la respuesta contiene datos
      first_proposal <- dades[1,]  # Tomar la primera propuesta
      data$latitude[i] <- first_proposal$latitude
      data$longitude[i] <- first_proposal$longitude
    } else {
      print(sprintf("No data returned for row %s: city = %s, country = %s", i, city, country))
    }
  } else {
...
if (status_code(response) == 200) {
  ...
  if (length(dades) > 0) {  # Comprobar que la respuesta contiene datos
    first_proposal <- dades[1,]  # Tomar la primera propuesta
    print(first_proposal$latitude)
    print(first_proposal$longitude)
  }
} else {
  print(sprintf("Error en fila %s: %s %s", i, status_code(response), content(response, "text", encoding = "UTF-8")))
}
...
for (i in 1:nrow(data)) {
  ...
  Sys.sleep(1)  # Pausa de 1 segundo entre cada solicitud
...
# Realizar la solicitud GET
response <- GET(url = api_url, add_headers(`X-Api-Key` = 'XAv1H8bp0iD+snw9znO3XA==g8KXgtstezF2zl2A'))

# Verificar el estado de la respuesta y manejar el contenido
if (status_code(response) == 200) {
  # Si la respuesta es exitosa, imprimir el contenido
  print(content(response, "text"))
} else {
  # Si hay un error, imprimir el código de estado y el mensaje de error
  print(sprintf("Error: %s %s", status_code(response), content(response, "text")))
}
...
# Verificar el estado de la respuesta y procesar el JSON
if (status_code(response) == 200) {
  # Convertir la respuesta JSON en una lista de R
  dades <- fromJSON(content(response, "text", encoding = "UTF-8"))
  print(dades)
  # Acceder a la primera propuesta y extraer latitud y longitud
  first_proposal <- dades[1,]
  print(first_proposal)
  latitude <- first_proposal$latitude
  longitude <- first_proposal$longitude
  print(latitude)
  print(paste("Latitud:", latitude, "Longitud:", longitude))
} else {
  # Imprimir el código de estado y el mensaje de error
  print(sprintf("Error: %s %s", status_code(response), content(response, "text", encoding = "UTF-8")))
}
